	...
	if(argc != 4){
      ...
      for(int i = 0; i < xres * yres; i++){
        ...
        pixels[i].b = 0;
			...
			// Calculate the e1, e2, and e3 basis vectors
			Vector3d e1;
			Vector3d e2;
			Vector3d e3;
			...
			// Must cudamalloc object copies, lights, camera, pixels
			// Must write kerneL that, for each pixel, calculates lighting based on normal
			object *dev_object_copies;
			cudaMalloc((void **)&dev_object_copies, sizeof(object) * object_copies.size());
			cudaMemcpy(dev_object_copies, object_copies, sizeof(object) * object_copies.size(), cudaMemcpyHostToDevice);

			light *dev_lights;
			cudaMalloc((void **)&dev_lights, sizeof(light) * lights.size());
			cudaMemcpy(dev_lights, lights, sizeof(light) * lights.size(), cudaMemcpyHostToDevice);

			camera *dev_camera;
			cudaMalloc((void **)&dev_camera, sizeof(camera));
			cudaMemcpy(dev_camera, c, sizeof(camera), cudaMemcpyHostToDevice);
      ...
      cudaCallRaytraceKernel(blocks, threadsPerBlock, object_copies, lights, c, pixels, e1, e2, e3, xres, yres);
      ...
      cout<<"P3"<<endl;
			...
			for(int i = 0; i < yres; i++){
				for(int j = 0; j < xres; j++){
					cout<<(int)(dev_pixels[i * xres + j].r)<<" "<<(int)(dev_pixels[i * xres + j].g)<<" "<<(int)(dev_pixels[i * xres + j].b)<<endl;
				}
			}
			cudaFree(dev_object_copies);
			cudaFree(dev_lights);
			cudaFree(dev_camera);
			cudaFree(dev_pixels);
    }
